...
class TestDataPreparation:
    ...
    def test_prepare_data_from_cache(self):
        ...
        # Mock the load_pickled_data function from eq.utils.common
        with patch('eq.utils.common.load_pickled_data') as mock_load:
            ...
            with patch('cv2.imwrite') as mock_imwrite:
                ...
                with patch('pathlib.Path.mkdir') as mock_mkdir:
                    ...
                    with patch('eq.segmentation.fastai_segmenter.DataBlock') as mock_datablock:
                        ...
                        # Set the length directly on the mock objects
                        type(mock_dls.train_ds).__len__ = MagicMock(return_value=10)
                        type(mock_dls.valid_ds).__len__ = MagicMock(return_value=5)
...
class TestTrainingPipeline:
    ...
    def test_mode_specific_callbacks_production(self):
        """Test callback creation for production mode."""
        config = SegmentationConfig(device_mode="production")
        segmenter = FastaiSegmenter(config)
        
        callbacks = segmenter._get_mode_specific_callbacks()
        
        # Should have at least 3 callbacks: EarlyStopping + 2 production-specific
        assert len(callbacks) >= 3
        
        # Check that production-specific callbacks are present
        callback_names = [type(cb).__name__ for cb in callbacks]
        assert 'EarlyStoppingCallback' in callback_names
    
    def test_mode_specific_callbacks_development(self):
        """Test callback creation for development mode."""
        config = SegmentationConfig(device_mode="development")
        segmenter = FastaiSegmenter(config)
        
        callbacks = segmenter._get_mode_specific_callbacks()
        
        # Should have at least 3 callbacks: EarlyStopping + 2 development-specific
        assert len(callbacks) >= 3
        
        # Check that development-specific callbacks are present
        callback_names = [type(cb).__name__ for cb in callbacks]
        assert 'EarlyStoppingCallback' in callback_names
    
    def test_mode_specific_callbacks_auto(self):
        """Test callback creation for auto mode."""
        config = SegmentationConfig(device_mode="auto")
        segmenter = FastaiSegmenter(config)
        
        callbacks = segmenter._get_mode_specific_callbacks()
        
        # Should have at least 1 callback: EarlyStopping
        assert len(callbacks) >= 1
        
        # Check that basic callbacks are present
        callback_names = [type(cb).__name__ for cb in callbacks]
        assert 'EarlyStoppingCallback' in callback_names
    
    def test_training_results_extraction(self):
        """Test extraction of training results."""
        config = SegmentationConfig()
        segmenter = FastaiSegmenter(config)
        
        # Mock learn object with recorder
        mock_learn = MagicMock()
        mock_learn.recorder.values = [[0.5, 0.6, 0.7]]  # Mock training history
        mock_learn.recorder.lrs = [1e-3, 1e-4, 1e-5]  # Mock learning rates
        segmenter.learn = mock_learn
        segmenter.device = torch.device("cpu")
        
        results = segmenter._extract_training_results()
        
        assert 'training_loss' in results
        assert 'validation_loss' in results
        assert 'dice_score' in results
        assert 'history' in results
        assert 'learning_rates' in results
        assert 'device_used' in results
        assert 'mode_used' in results
        assert results['device_used'] == 'cpu'
        assert results['mode_used'] == 'auto'
    
    def test_production_mode_training(self):
        """Test production mode training strategy."""
        config = SegmentationConfig(device_mode="production")
        segmenter = FastaiSegmenter(config)
        
        # Mock learn object
        mock_learn = MagicMock()
        mock_learn.recorder.values = [[0.3, 0.4, 0.9]]  # Mock training history
        segmenter.learn = mock_learn
        
        result = segmenter._train_production_mode(10, 1e-3, [])
        
        assert 'training_loss' in result
        assert 'validation_loss' in result
        assert 'dice_score' in result
        assert result['mode_used'] == 'production'
    
    def test_development_mode_training(self):
        """Test development mode training strategy."""
        config = SegmentationConfig(device_mode="development")
        segmenter = FastaiSegmenter(config)
        
        # Mock learn object
        mock_learn = MagicMock()
        mock_learn.recorder.values = [[0.2, 0.3, 0.95]]  # Mock training history
        segmenter.learn = mock_learn
        
        result = segmenter._train_development_mode(15, 1e-4, [])
        
        assert 'training_loss' in result
        assert 'validation_loss' in result
        assert 'dice_score' in result
        assert result['mode_used'] == 'development'
    
    def test_auto_mode_training(self):
        """Test auto mode training strategy."""
        config = SegmentationConfig(device_mode="auto")
        segmenter = FastaiSegmenter(config)
        
        # Mock learn object
        mock_learn = MagicMock()
        mock_learn.recorder.values = [[0.4, 0.5, 0.85]]  # Mock training history
        segmenter.learn = mock_learn
        
        result = segmenter._train_auto_mode(20, 1e-3, [])
    ...
    def test_training_with_learning_rate_scheduling(self):
        ...
        # Should have learning rate scheduling callback
        callback_names = [type(cb).__name__ for cb in callbacks]
        assert 'ReduceLROnPlateau' in callback_names
    
    def test_training_with_model_saving(self):
        """Test training with model saving callbacks."""
        config = SegmentationConfig(device_mode="development")
        segmenter = FastaiSegmenter(config)
        
        callbacks = segmenter._get_mode_specific_callbacks()
        
        # Should have model saving callback
        callback_names = [type(cb).__name__ for cb in callbacks]
        assert 'SaveModelCallback' in callback_names
