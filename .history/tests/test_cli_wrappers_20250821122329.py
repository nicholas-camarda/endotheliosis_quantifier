...
def test_runtime_check_wrapper():
    ...
    from runtime_check import check_imports, check_data_paths
...
def test_smoke_test_pipeline_wrapper():
    ...
    from smoke_test_pipeline import run_smoke_test
...
def test_eq_package_imports():
    ...
    modules_to_test = [
        ...
        'eq.pipeline.quantify_endotheliosis',
...
def test_wrapper_function_calls():
    ...
    # Test run_random_forest through wrapper
    with tempfile.TemporaryDirectory() as temp_dir:
        X_train = np.random.rand(10, 5)  # Small test data
        y_train = np.random.uniform(0, 3, 10)
        X_val = np.random.rand(5, 5)
        y_val = np.random.uniform(0, 3, 5)
        ...
        try:
            ...
            print("✓ run_random_forest works through eq package")
        except Exception as e:
            # Should not fail on basic functionality
            assert "eq" not in str(e), f"run_random_forest failed: {e}"
