...
from eq.config.mode_manager import ModeManager
...
class TestThreeStagePipeline:
    ...
    def test_quant_endo_command_structure(self, temp_dirs, mock_environment):
        ...
        with patch('eq.segmentation.fastai_segmenter.FastaiSegmenter') as mock_segmenter, \
             patch('eq.utils.output_manager.OutputManager') as mock_output_manager, \
             patch('eq.features.data_loader.DataLoader') as mock_data_loader, \
             patch('eq.pipeline.quantify_endotheliosis.QuantificationModel') as mock_quant_model:
            ...
            # Mock quantification model
            mock_quant_model_instance = MagicMock()
            mock_quant_model.return_value = mock_quant_model_instance
    ...
    def test_production_command_structure(self, temp_dirs, mock_environment):
        ...
        with patch('eq.segmentation.fastai_segmenter.FastaiSegmenter') as mock_segmenter, \
            ...
            # Mock quantification model
            mock_quant_model_instance = MagicMock()
            mock_quant_model.load_model.return_value = mock_quant_model_instance
    ...
    def test_orchestrator_menu_options(self):
        ...
        with patch('builtins.input', return_value='4'):
            with patch('builtins.print') as mock_print:
                ...
                assert "Segmentation Training (seg)" in menu_text
                assert "Quantification Training (quant-endo)" in menu_text
                assert "Production Inference (production)" in menu_text
                assert "Smoke Test" in menu_text
