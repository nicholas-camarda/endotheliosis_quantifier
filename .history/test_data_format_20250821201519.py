...
def test_raw_data():
    ...
    try:
        ...
        train_masks = np.load(cache_dir / 'train_masks.npy')
        val_images = np.load(cache_dir / 'val_images.npy')
        val_masks = np.load(cache_dir / 'val_masks.npy')
        ...
        print(f"   Train image sample: {train_images[0, :5, :5, 0].flatten()[:10]}")
        print(f"   Train mask sample: {train_masks[0, :5, :5, 0].flatten()[:10]}")
...
def create_fixed_visualization(train_images, train_masks, val_images, val_masks):
    ...
    for i in range(n_samples):
        ...
        img = val_images[i, :, :, 0]  # Remove channel dimension
        ...
        mask = val_masks[i, :, :, 0]  # Remove channel dimension
