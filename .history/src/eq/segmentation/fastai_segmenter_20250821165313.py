...
class FastaiSegmenter:
    ...
    def _setup_device(self) -> torch.device:
        """Setup device based on hardware capabilities and mode."""
        backend, explanation = get_device_recommendation(self.config.device_mode)
        
        if backend.value == "mps":
            device = torch.device("mps")
            # Enable MPS fallback for CPU operations
            os.environ["PYTORCH_ENABLE_MPS_FALLBACK"] = "1"
        elif backend.value == "cuda":
            device = torch.device("cuda")
        else:
            device = torch.device("cpu")
            
        print(f"Using device: {device} ({explanation})")
        return device
    ...
    def prepare_data_from_cache(self, cache_dir: Path, task_type: str) -> None:
        ...
        for i, (img, mask) in enumerate(zip(train_images, train_masks)):
            ...
            mask_path = temp_dir / f"train_mask_{i}.png"
            
            # Save image (assuming it's in the right format)
            import cv2
        ...
        for i, (img, mask) in enumerate(zip(val_images, val_masks)):
            ...
            mask_path = temp_dir / f"val_mask_{i}.png"
        ...
        from fastai.data.core import DataBlock, Splitter
        ...
        data_block = DataBlock(
            ...
            get_y=lambda x: x.replace('.jpg', '_mask.png'),
            item_tfms=[RandomResizedCrop(self.config.image_size, min_scale=self.config.min_scale)],
    ...
    def _get_advanced_augmentations(self):
        ...
        return [
            *aug_transforms(
            ...
            RandomBrightness(max_lighting=0.2, p=0.5),
            RandomContrast(max_lighting=0.2, p=0.5),
            RandomGamma(p=0.5),
    ...
    def predict(self, image_path: Union[str, Path]) -> np.ndarray:
        ...
        # Load and predict
        pred = self.learn.predict(image_path)
