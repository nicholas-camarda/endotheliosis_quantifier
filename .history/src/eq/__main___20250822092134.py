...
# Custom functions for fastai model loading
def n_glom_codes():
    """Number of glomeruli class codes."""
    return 2

def get_glom_mask_file(image_path):
    """Get glomeruli mask file path."""
    return str(image_path).replace('.jpg', '_mask.png')

def get_glom_y(image_path):
    """Get glomeruli mask path for fastai."""
    return str(image_path).replace('.jpg', '_mask.png')

def get_y(image_path):
    """Get mask path for fastai (generic function for different models)."""
    return str(image_path).replace('.jpg', '_mask.png')
