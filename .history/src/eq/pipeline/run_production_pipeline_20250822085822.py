...
def run_pipeline(epochs: int = 10, run_type: str = "production", use_existing_models: bool = True):
    ...
    if use_existing_models:
        ...
        # If we have models, use them for inference instead of training
        if any(model_path.exists() for model_path in existing_models.values()):
            print("\n🎯 Using existing pre-trained models for inference...")
            run_inference_with_existing_models(existing_models, output_dirs, run_type)
            return True
    
    # Fall back to training if no models or use_existing_models=False
    print("\n🏋️ Training new models (no existing models found or training mode requested)...")
...
def run_inference_with_existing_models(existing_models: Dict[str, Path], output_dirs: Dict[str, Path], run_type: str):
    ...
    if existing_models['glomeruli'].exists():
        ...
        config = SegmentationConfig(
            device_mode='development',
            model_save_path=output_dirs['models'] / f"{run_type}_glomerulus_segmenter.pkl",
            results_save_path=output_dirs['results']
        )
        ...
        run_info = {
            ...
            'run_type': f"{run_type}_inference",
            'config': {
        ...
        print(f"\n🎉 === {run_type.upper()} INFERENCE COMPLETE ===")
        print(f"📁 All outputs saved to: {output_dirs['main']}")
