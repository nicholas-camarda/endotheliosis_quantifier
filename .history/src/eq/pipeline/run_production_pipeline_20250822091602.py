...
from eq.segmentation.config import SegmentationConfig
...
def run_pipeline(epochs: int = 50, run_type: str = "production", use_existing_models: bool = True, 
    ...
    # Set environment for MPS compatibility - force CPU fallback for problematic operations
    ...
    existing_models = {
        'segmentation': Path(segmentation_model_path)
    }
...
def run_full_production_inference(output_dirs: Dict[str, Path], existing_models: Dict[str, Path], data_dir: str, cache_dir: str, config_manager: PipelineConfigManager):
