...
def test_path_helpers_resolve_from_repo_root(monkeypatch):
    ...
    assert (
        get_runtime_cohort_manifest_path()
        == active_root / 'raw_data/cohorts/manifest.csv'
    )
    assert (
        get_runtime_cohort_manifest_summary_path()
        == active_root / 'derived_data/cohort_manifest/manifest_summary.json'
    )
    assert (
        get_runtime_cohort_path('vegfri_dox')
        == active_root / 'raw_data/cohorts/vegfri_dox'
    )
    assert (
        get_runtime_mitochondria_data_path()
        == active_root / 'raw_data/mitochondria_data'
    )
    assert (
        get_runtime_segmentation_evaluation_root()
        == active_root / 'output/segmentation_evaluation'
    )
    assert (
    ...
    assert (
        get_runtime_prediction_path('glomeruli')
        == active_root / 'output/predictions/glomeruli'
    )
    assert (
        get_runtime_quantification_results_root()
        == active_root / 'output/quantification_results'
    )
    assert (
        get_runtime_quantification_result_path('lauren_preeclampsia')
        == active_root / 'output/quantification_results/lauren_preeclampsia'
    )
...
def test_path_helpers_prefer_runtime_root_when_present(tmp_path, monkeypatch):
    ...
    assert (
        get_runtime_cohort_path('lauren_preeclampsia')
        == runtime_root / 'raw_data/cohorts/lauren_preeclampsia'
    )
    assert (
        get_runtime_mitochondria_data_path()
    ...
    assert (
        get_runtime_segmentation_evaluation_root()
        == runtime_root / 'output/segmentation_evaluation'
    )
    assert (
        get_runtime_segmentation_evaluation_path('glomeruli_candidate_comparison')
        == runtime_root
    ...
    assert (
        get_runtime_prediction_path('mitochondria')
        == runtime_root / 'output/predictions/mitochondria'
    )
    assert (
        get_runtime_quantification_results_root()
        == runtime_root / 'output/quantification_results'
    )
    assert (
        get_runtime_quantification_result_path('lauren_preeclampsia')
        == runtime_root / 'output/quantification_results/lauren_preeclampsia'
    )
...
def test_runtime_cohort_helpers_accept_explicit_runtime_root(tmp_path):
    ...
    assert (
        get_runtime_cohort_manifest_path(runtime_root)
        == runtime_root / 'raw_data/cohorts/manifest.csv'
    )
    assert (
        get_runtime_cohort_manifest_summary_path(runtime_root)
        == runtime_root / 'derived_data/cohort_manifest/manifest_summary.json'
    )
    assert (
        get_runtime_cohort_path('vegfri_dox', runtime_root)
        == runtime_root / 'raw_data/cohorts/vegfri_dox'
    )
    assert (
        get_runtime_mitochondria_data_path(runtime_root)
        == runtime_root / 'raw_data/mitochondria_data'
    )
    assert (
        get_runtime_segmentation_evaluation_root(runtime_root)
        == runtime_root / 'output/segmentation_evaluation'
    )
    assert (
    ...
    assert (
        get_runtime_predictions_root(runtime_root)
        == runtime_root / 'output/predictions'
    )
    assert (
        get_runtime_prediction_path('glomeruli', runtime_root)
        == runtime_root / 'output/predictions/glomeruli'
    )
    assert (
        get_runtime_quantification_results_root(runtime_root)
        == runtime_root / 'output/quantification_results'
    )
    assert (
        get_runtime_quantification_result_path('vegfri_dox', runtime_root)
        == runtime_root / 'output/quantification_results/vegfri_dox'
    )
...
def test_config_manager_uses_resolved_paths_and_reloadable_global_config(
...
def test_output_manager_cleanup_uses_run_metadata_created_at(tmp_path):
    ...
    (stale_dirs['main'] / 'run_metadata.json').write_text(
        json.dumps({'created_at': old_created_at}, indent=2), encoding='utf-8'
    )
    (fresh_dirs['main'] / 'run_metadata.json').write_text(
        json.dumps({'created_at': new_created_at}, indent=2), encoding='utf-8'
    )
