    ...
    def test_train_model_glomeruli(self):
        ...
        try:
            ...
            try:
                ...
                with patch('eq.pipeline.segmentation_pipeline.train_glomeruli_transfer_learning_from_config') as mock_train:
                    mock_train.return_value = None  # Return None for testing
                    pipeline._train_model()
                    
                # Verify training function was called with correct arguments
                mock_train.assert_called_once()
                call_args = mock_train.call_args[0]  # First argument is config_path
                assert call_args[0] == temp_file  # Should be called with config file path
            finally:
                os.unlink(temp_file)
        finally:
