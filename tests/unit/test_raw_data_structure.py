...
def test_preeclampsia_project_structure_if_present():
    raw_data_root = _get_raw_data_root()
    project_dir = raw_data_root / "preeclampsia_project"
    if not project_dir.exists():
        pytest.skip(f"Preeclampsia project not present under {raw_data_root}")
...
def test_subject_metadata_is_readable_if_present():
    raw_data_root = _get_raw_data_root()
    metadata_file = raw_data_root / "preeclampsia_project" / "subject_metadata.xlsx"
...
def test_raw_data_contains_nonempty_images_if_present():
    raw_data_root = _get_raw_data_root()
    image_root = raw_data_root / "preeclampsia_project" / "images"
