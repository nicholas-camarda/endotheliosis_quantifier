...
class TestPatchifyImages:
    ...
    def test_patchify_various_image_formats(self, temp_dir):
        ...
        # Create images in different formats with different base names to avoid conflicts
        formats = [('.jpg', 'test1'), ('.jpeg', 'test2'), ('.png', 'test3'), ('.tif', 'test4')]
        for fmt, base_name in formats:
            self.create_test_image((16, 16), f"{base_name}{fmt}", str(input_dir))
    ...
    def test_patchify_preserve_original_extension(self, temp_dir):
        ...
        # Create images with different extensions and base names to avoid conflicts
        extensions = [('.jpg', 'test1'), ('.jpeg', 'test2'), ('.png', 'test3'), ('.tif', 'test4')]
        for ext, base_name in extensions:
            self.create_test_image((16, 16), f"{base_name}{ext}", str(input_dir))
        
        # Patchify
        patchify_image_dir(8, str(input_dir), str(output_dir))
        
        # Check that extensions are preserved in patch names
        for ext, base_name in extensions:
            ext_name = ext[1:]  # Remove the dot
            patches = list(output_dir.glob(f"{base_name}_*.jpg"))
            assert len(patches) == 4
            
            # Verify patch names contain original extension identifier
            for patch_path in patches:
                assert f"{base_name}_" in patch_path.name
