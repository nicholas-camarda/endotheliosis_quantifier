...
def test_unet_learner_initializes_with_v2_dls(tmp_path: Path):
    ...
    dls = build_segmentation_dls_dynamic_patching(tmp_path, bs=4, num_workers=0, crop_size=64)
    learn = unet_learner(dls, resnet18, n_out=2, pretrained=False)
...
def test_minimal_training_step_with_v2_dls(tmp_path: Path):
    ...
    dls = build_segmentation_dls_dynamic_patching(tmp_path, bs=2, num_workers=0, crop_size=64)
    learn = unet_learner(dls, resnet18, n_out=2, pretrained=False)
