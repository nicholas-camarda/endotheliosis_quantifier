...
- Store cohort-owned runtime inputs and manifest state under the active runtime root, not under the mostly empty repo-local `data/` placeholders on this machine. In normal use that means `$EQ_RUNTIME_ROOT/raw_data/cohorts/<cohort_id>/` for localized cohort datasets, `$EQ_RUNTIME_ROOT/raw_data/cohorts/manifest.csv` for the canonical dataset-wide manifest, and `$EQ_RUNTIME_ROOT/output/cohorts/<cohort_id>/` for model/run artifacts.
...
- Affected artifacts: one unified runtime manifest, localized cohort directories containing organized runtime images/masks/scores/metadata, mapping-verification fields, segmentation transport audit reports, predicted ROI images/masks, score-linked grading tables, and admission/exclusion states under the active runtime root: `$EQ_RUNTIME_ROOT/raw_data/cohorts/manifest.csv`, `$EQ_RUNTIME_ROOT/raw_data/cohorts/<cohort_id>/`, and `$EQ_RUNTIME_ROOT/output/cohorts/<cohort_id>/`.
