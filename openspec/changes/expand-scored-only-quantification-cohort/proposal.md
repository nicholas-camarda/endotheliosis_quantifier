...
- Store cohort-owned runtime inputs and manifest state under the active runtime root, not under the repo-local `data/` tree. In normal use that means `$EQ_RUNTIME_ROOT/raw_data/cohorts/<cohort_id>/` for localized cohort datasets and `$EQ_RUNTIME_ROOT/raw_data/cohorts/manifest.csv` for the canonical dataset-wide manifest. Rebuildable model/run artifacts are split by result family under `$EQ_RUNTIME_ROOT/output/segmentation_results/` and `$EQ_RUNTIME_ROOT/output/quantification_results/`.
...
- Affected artifacts: one unified runtime manifest, localized cohort directories containing organized runtime images/masks/scores/metadata, mapping-verification fields, segmentation transport audit reports, predicted ROI images/masks, score-linked grading tables, and admission/exclusion states under the active runtime root: `$EQ_RUNTIME_ROOT/raw_data/cohorts/manifest.csv`, `$EQ_RUNTIME_ROOT/raw_data/cohorts/<cohort_id>/`, `$EQ_RUNTIME_ROOT/output/segmentation_results/`, and `$EQ_RUNTIME_ROOT/output/quantification_results/`.
