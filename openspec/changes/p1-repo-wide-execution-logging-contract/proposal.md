...
- Require an execution-surface inventory that classifies every live `eq` subcommand and direct `python -m eq...` module as automatic runtime logging, explicit `--log-file` only, function-event-only, low-level helper, retired, or intentionally unsupported.
...
- Add tests that prove logging behavior across the full supported ecosystem: all six committed `eq run-config` workflows, direct workflow module execution, direct training module execution, quantification workflow execution, imported high-level function calls, subprocess tee capture, failure capture, and handler cleanup.
...
- Runtime log paths resolve through canonical runtime/log helpers in `src/eq/utils/paths.py`, including `get_logs_path()`, rather than creating a parallel path system.
...
- Direct-run ID derivation order is `--run-id` when present, config `run.name` when present, output-folder or model-run stem when present, otherwise a generated timestamp ID.
