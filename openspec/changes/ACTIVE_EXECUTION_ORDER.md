...
| 1 | `oracle-current-docs-quarantine` | archived `2026-04-29-oracle-current-docs-quarantine` | `README.md`, `docs/README.md`, `docs/INTEGRATION_GUIDE.md`, `docs/PIPELINE_INTEGRATION_PLAN.md`, `docs/SEGMENTATION_ENGINEERING_GUIDE.md`, `docs/ONBOARDING_GUIDE.md`, `docs/TECHNICAL_LAB_NOTEBOOK.md`, `docs/HISTORICAL_NOTES.md`, `docs/archive/`, docs checks | Complete. Later changes must preserve the current-doc/archive boundary. |
| 2 | `p1-align-fastai-practices-and-archive-history` | archived `2026-04-29-p1-align-fastai-practices-and-archive-history` | `src/eq/data_management/datablock_loader.py`, `src/eq/training/train_glomeruli.py`, `src/eq/training/train_mitochondria.py`, `src/eq/training/transfer_learning.py`, `src/eq/utils/run_io.py`, FastAI docs quarantine checks | Complete. Later changes must reuse the explicit-import, manifest-pairing, required-artifact, and trusted-learner contracts. |
| 3 | `oracle-canonical-quantification-input-contract` | archived `2026-04-29-oracle-canonical-quantification-input-contract` | `configs/endotheliosis_quantification.yaml`, `src/eq/__main__.py`, `src/eq/run_config.py`, `src/eq/quantification/cohorts.py`, `src/eq/quantification/labelstudio_scores.py`, `src/eq/quantification/pipeline.py`, `src/eq/quantification/run_endotheliosis_quantification_workflow.py`, `src/eq/quantification/endotheliosis_grade_model.py` | Complete. Later changes must consume `src/eq/quantification/input_contract.py` and preserve the stable reviewed-label override path. |
| 4 | `oracle-harden-audit-contracts` | archived `2026-04-29-oracle-harden-audit-contracts` | Label Studio score loading, manifest training pairing, preprocessing, ROI extraction, candidate comparison, exact artifact handoffs, shared quantification modeling contracts, CLI startup/MPS scope | Complete. Later changes must reuse the hardened fail-closed label, ROI, embedding, artifact-handoff, startup, and modeling contracts. |
| 5 | `label-free-roi-embedding-atlas` | archived `2026-04-29-label-free-roi-embedding-atlas` | `configs/label_free_roi_embedding_atlas.yaml`, `src/eq/run_config.py`, `src/eq/__main__.py`, `src/eq/quantification/embedding_atlas.py`, `src/eq/quantification/embeddings.py`, `src/eq/quantification/learned_roi.py`, `src/eq/quantification/modeling_contracts.py`, docs/tests | Complete. The configured production atlas now fails closed until upstream quantification artifacts carry the required hardened ROI/embedding provenance. |
