...
- Affected CLI/API surfaces: `eq --help`, `eq quant-endo`, `eq prepare-quant-contract`, `eq cohort-manifest`, `eq run-config`, `eq capabilities`, `eq mode`, direct module commands `python -m eq.training.train_mitochondria`, `python -m eq.training.train_glomeruli`, `python -m eq.training.run_segmentation_fixedloader_full`, and direct or documented commands discovered during the audit.
