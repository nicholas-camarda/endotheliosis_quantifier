...
- logging-contract: future execution-surface changes must state whether the surface is `entrypoint_capture`, `global_log_file_only`, `subprocess_worker`, `high_level_function_events_only`, `low_level_helper_no_durable_logging`, or `retired_or_unsupported`.
- docs-impact: future execution-surface changes must state whether README/operator docs need log-location updates or why the existing `logs/run_config`, `logs/direct`, and `eq --log-file` guidance remains complete.
- lint-scope: repository-wide lint validation is part of completion; the implementation removes lint blockers that could mask future logging-contract regressions, including hidden FastAI star-import names, duplicate runtime path helper definitions, bare exception handlers, and import-order drift.
