...
class ModeManager:
    ...
    def validate_mode(self, mode: EnvironmentMode) -> Tuple[bool, str]:
        ...
        if mode == EnvironmentMode.PRODUCTION:
            ...
            # Tightened heuristic: only allow production on CUDA, or MPS with >= 32GB unified memory
            if hardware_capabilities.backend_type:
                backend_val = hardware_capabilities.backend_type.value
                if backend_val == "mps" and hardware_capabilities.total_memory_gb < 32:
                    return False, "Production on MPS requires >= 32GB unified memory"
                if backend_val == "cuda":
                    return True, "Production mode validated for CUDA backend"
            # If not covered above, default to development recommendation
            return False, "Production mode restricted to CUDA or MPS with >= 32GB"
...
"""
    ...
    def get_device_recommendation(self) -> str:
        """
        Get the device recommendation for the current mode.
        
        Returns:
            Device recommendation string
        """
        from eq.utils.hardware_detection import get_device_recommendation
        
        backend, explanation = get_device_recommendation(self.current_config.device_mode)
        return f"{backend.value.upper()} - {explanation}"
